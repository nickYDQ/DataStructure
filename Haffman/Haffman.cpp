...
//	for (int i = 1; i < totalsize - 1; i++)
//	{
//		for (int j = i; j >= 1 && info_nodes[j].num > info_nodes[j - 1].num; j--)//对for循环的理解！
//		{
//			swap(info_nodes[j], info_nodes[j - 1]);
//		}
//	}
...
void getNodes_code(){
	int count = 0;//用于记录现在进行了多少次"合并"
	int min1 = 0, min2 = 0;//用于存贮两个最小的节点的下标

	while (count != kinds_num - 1){//有kinds_num个节点,需要kinds_num-2次合并

		//查找值最小的一个节点
		min1 = totalsize * 2 - 1;
		info_nodes[min1].num = 1000000;
		for (int j = 0; j < kinds_num + count; j++)
		{
		...
		//将这两个节点指向其父亲节点，其父亲节点的位置在kinds_num + count处，这里你不懂再问我吧
		info_nodes[kinds_num + count].num = info_nodes[min1].num + info_nodes[min2].num;

		info_nodes[kinds_num + count].rchild = &info_nodes[min1];
		info_nodes[kinds_num + count].right_int = min1;

		info_nodes[kinds_num + count].lchild = &info_nodes[min2];
		info_nodes[kinds_num + count].left_int = min2;

		info_nodes[min1].Father = &info_nodes[kinds_num + count];
		info_nodes[min2].Father = &info_nodes[kinds_num + count];
		info_nodes[min1].Father_int = kinds_num + count;
		info_nodes[min2].Father_int = kinds_num + count;
		info_nodes[kinds_num + count].isable = true;
		count++;
	...
	if (kinds_num != 1)
	{
		BTNode *node = &info_nodes[kinds_num - 1 + count];
		node->Father = NULL;
		PreOrderTravel(node);
	}
	else
	...
	for (int count1 = 0; count1 < totalsize * 2 - 1; count1++){
		if (info_nodes[count1].percent != 0)
		{
...
void decodeFromFile()
{
	...
	getline(code, str);
	sp_num_truesize = std::atoi(str.c_str());;
	getline(code, str);
	kinds_num = std::atoi(str.c_str());
	getline(code, str);
	sp_num_tohead = std::atoi(str.c_str());
	...
	while (getline(code, str))
	{
		...
		if (i == 2)
		{
			info_nodes[j].Father_int = std::atoi(str.c_str());
		...
		if (i == 3)
		{
			info_nodes[j].left_int = std::atoi(str.c_str());
		...
		if (i == 4)
		{
			info_nodes[j].right_int = std::atoi(str.c_str());
